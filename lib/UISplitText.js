 ...
 * 		<br>
 * 		<img src="https://github.com/ff0000-ad-tech/ad-docs/blob/master/assets/ad-ui/breakSpace_true.jpg" />
 * 		<br>
 * 		<br>
 * 		<img src="https://github.com/ff0000-ad-tech/ad-docs/blob/master/assets/ad-ui/breakSpace_false.jpg" />
 * 		<br>
...
export default class UISplitText {
	constructor(arg) {
		...
		const _spacing = (_spaceTextDouble.width - _spaceTextSingle.width) * (arg.splitWordSpacing || 1) //Math.ceil(_mainTF.fontSize / (3.7 / (arg.padding || arg.spacing || 1)));
		...
		for (let i = 0; i < _parsedWords.length; i++) {
			...
			if (_word) {
				...
				if (
					_wordHasBreak ||
					(_mainTF.format.indexOf('paragraph') >= 0 && _newWord.width + _newWord.x > _mainTF.width)
				) {
					_lineNum++
		...
		for (let line in _textLines) {
			...
			if (arg.allowSpaceOnBreaks && _lastChar.match(new RegExp(arg.spaceBreakIncludes || 'a-z0-9]', 'gi')))
				_group.x -= _spacing
		...
		if (_splitType === TextSplitType.CHARACTERS) {
			...
			if (arg.allowSpaceOnBreaks)
				for (let l = 0; l < _textContainer.lines.length; l++) Clamp.set(_textContainer.lines[l], Clamp.XY)
		}
		...
		if (align) Align.set(_textContainer, align)
