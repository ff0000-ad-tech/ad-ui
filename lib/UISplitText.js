...
UISplitText.prototype = {
	_init: function(arg) {
		...
		while (_spanIndexStart >= 0) {
			...
			trace(_mySplitText);
			trace(0, _mySplitText.indexOf('>') + 1);
		...
		for (var line in _textLines) {
			...
			if (arg.allowSpaceOnBreaks && _lastChar.match(new RexExp(arg.spaceBreakIncludes || 'a-z0-9]', 'gi'))) _group.x -= _spacing;
		...
		if (_splitType === TextSplitType.CHARACTERS) {
			...
			if (arg.allowSpaceOnBreaks)
				for (var l = 0; l < _textContainer.lines.length; l++) Clamp.set(_textContainer.lines[l], Clamp.XY);
		}
