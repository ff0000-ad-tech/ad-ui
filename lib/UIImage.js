 ...
 * @class UIImage
 * @desc
 * This is a display object class, extending {@link UIComponent}.  It is a DOM element that has default values for the
 * background image styles. They can still be overwritten by simply changing them with {@link Styles.setCss} or the
 * native {@link UIComponent.setCss} method directly on the UIImage instance.<br>
...
class UIImage {
  constructor(arg) {
    ...
    // MARKUP
    if (!arg.source)
      throw new Error("UIImage : No image source set on '" + arg.id + "'");
    ...
    Object.defineProperties(U, {
      ...
      source: {
        ...
        set: function(value) {
          let src;
          if (isDataURL(value)) {
            // TODO: get width + height of base64 image
            const srcWithoutNewlines = value.replace(/(\r\n|\n|\r)/gm, "");
            _source = src = srcWithoutNewlines;
          } else {
            _source = ImageManager.get(value);
            src = _source.src;
          }
          U.style.backgroundImage = "url(" + src + ")";
        }
      },
    ...
    // determine whether source url is a Data URL (i.e. "data:image,png;base64:...")
