 ...
 * See {@link UIComponent} for more info.<br>
	...
	constructor(arg) {
		...
		*/
		...
		Styles.injectStylesheet(
			...
			'position:relative; display:table-cell; cursor:default; pointer-events:none; line-height:100%; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none;  box-sizing: content-box;'
		)
		...
		function fontSizeCheckFn() {
			U.style.fontSize = fontSize + 'px'
			return U.scrollHeight <= U.offsetHeight && U.scrollWidth <= U.offsetWidth
		}

		function getOptimalFontSize(low, high) {
			// doing a check on the high font to prevent extra resizes
			// when highest font size already fits w/in container
			...
			if (midFits) {
				return getOptimalFontSize(mid, high)
			}
			// optimal font size b/w low and mid
			return getOptimalFontSize(low, mid)
		}
		...
		function autoSizeContent() {
			...
			if (_format == 'paragraphFit') {
				var optimalFontSize = getOptimalFontSize(_minFontSize, _fontSize)
				...
				const large = 243
				const small = Device.browser == 'ie' ? 182 : _minFontSize
